...
#print(data.keys())
print(data.describe())
numeric_col=['Innings Minutes Batted','Innings Balls Faced','Innings Batting Strike Rate']
cor_matrix=data.loc[:,numeric_col].corr()
print(cor_matrix)
...
kohli=kohli[['Innings Runs Scored Num','Innings Minutes Batted','Innings Balls Faced','Innings Batting Strike Rate','Opposition']]


## Independent variables
chk_test=kohli[['Innings Balls Faced','Innings Minutes Batted',]].sample(50)
chk_train=kohli[['Innings Balls Faced','Innings Minutes Batted']].sample(50)
...
kohli_x_train = kohli[['Innings Balls Faced','Innings Minutes Batted']].sample(50)
kohli_z_train=kohli[['Innings Batting Strike Rate']].sample(50)
kohli_x_test = kohli[['Innings Balls Faced','Innings Minutes Batted']].sample(50)

#kohli_x_train = chk.values
#kohli_x_test = chk.values

kohli_y_train = kohli[['Innings Runs Scored Num']].sample(50)
kohli_y_test = kohli[['Innings Runs Scored Num']].sample(50)

model=linear_model.LinearRegression()
model2=linear_model.LinearRegression()
model2.fit(chk_train,kohli_y_train)
model.fit(kohli_x_train,kohli_y_train)
kohli_y_predicted=model.predict(kohli_x_test)
kohli_y_predicted2=model2.predict(chk_test)
...
print("Predicted score is: ",model2.predict([[a,b]]))

print("Batting avg:= ",statistics.mean(kohli['Innings Runs Scored Num']))
...
plt.bar(kohli['Innings Batting Strike Rate'],kohli['Innings Runs Scored Num'],color='maroon')
plt.xlabel("Strike Rate")
plt.ylabel("Innings Runs Scored")
plt.show()

plt.bar(kohli['Opposition'],kohli['Innings Runs Scored Num'],color='green')
plt.ylabel("Runs Scored")
plt.title("Runs scored against Oppositions")
